...
//Create software Serial1 object to communicate with GPS
SoftwareSerial gps( 9 , 8 );



void setup() {
  //Begin Serial1 comunication with Arduino and Arduino IDE (Serial1 Monitor)
//Serial1.begin(9600);
  //while(!Serial1);
  Serial1.begin(9600);
  while(!Serial1);
  ...
  Serial1.println("Setup Complete!");
    ...
    #endif
   //Being Serial1 communication witj Arduino and GPS Module //Important rate must be 9600
  gps.begin(9600);
  delay(1000);
    ...
    Serial1.print(accelgyro.getZAccelOffset()); Serial1.print("\t"); // 1688
    Serial1.print(accelgyro.getXGyroOffset()); Serial1.print("\t"); // 0
    Serial1.print(accelgyro.getYGyroOffset()); Serial1.print("\t"); // 0
    Serial1.print(accelgyro.getZGyroOffset()); Serial1.print("\t"); // 0
    Serial1.print("\n");
...
void loop() {
  //Read SIM800 output (if available) and print it in Arduino IDE Serial1 Monitor
  if(gps.available()){
  ...
  //Read Arduino IDE Serial1 Monitor inputs (if available) and send them to SIM800
